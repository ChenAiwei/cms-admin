...
layui.define(["element", "jquery", "okUtils", "okCookie"], function(exports) {
	...
	function createMenu(data, tabID) {
		if (data.children != undefined && data.children.length > 0) {
			...
			if (data.target) {
				temp += ('<a lay-id="{0}" data-url="{1}" target="{2}" is-close="{3}">').format(tabID, data.href, data.target,
					isClose);
			} else {
				...
				success: function(data) {
					if (data.code == 0) {
						...
						if (typeof _this.tabConfig.parseData == "function") {
							_data = _this.tabConfig.parseData(data);
						}
						...
						if ("Invalid token" == data.msg) {
							window.location = "../cms-admin/pages/login.html";
						}
					}
				}
			});
